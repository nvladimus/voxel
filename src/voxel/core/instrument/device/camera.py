...
type VoxelFrame = np.ndarray[np.uint8 | np.uint16]
...
class VoxelCamera[TriggerConfig](VoxelDevice):
    ...
    def __init__(self, name: str, pixel_size_um: tuple[float, float]) -> None:
        ...
        """
        super().__init__(name, device_type=VoxelDeviceType.CAMERA)
    ...
    @property
    @abstractmethod
    def trigger_settings(self) -> TriggerConfig:
        """
    ...
    @abstractmethod
    def trigger_settings(self, settings: TriggerConfig) -> None:
        """
