...
from voxel.core.instrument.daq.channel import DAQWaveform, VoxelDAQTaskChannel, VoxelDAQTaskTiming
...
if TYPE_CHECKING:
    from voxel.core.instrument.daq.ni import HardwareTask
...
class VoxelDAQTask(VoxelDevice):
    ...
    def add_channel(
        ...
        if (
            amplitude_volts > self.daq.ao_voltage_range[0]
            or center_volts - amplitude_volts < self.daq.ao_voltage_range[1]
        ):
            raise ValueError(
                f"Channel '{name}' exceeds the voltage range of the device. " f"Range: {self.daq.ao_voltage_range}"
            )
