...
class DAQTask(VoxelDevice):
    def __init__(self, name: str, task_type: DAQTaskType, sampling_frequency_hz: float,
                 period_time_ms: float, rest_time_ms: float, daq: 'VoxelNIDAQ') -> None:
    ...
    def add_channel(self, name: str, port: str, waveform_type: DAQWaveform,
                    center_volts: float, amplitude_volts: float,
                    start_time_ms: float, end_time_ms: float, cut_off_frequency_hz: float) -> DAQTaskChannel:
        ...
        channel = DAQTaskChannel(
            ...
            cut_off_frequency_hz=cut_off_frequency_hz,
            port=port,
