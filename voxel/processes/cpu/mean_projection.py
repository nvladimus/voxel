        ...
        while frame_index < self._frame_count_px_px:
            ...
            if self.new_image.is_set():
                ...
                if chunk_index == self._projection_count_px - 1 or frame_index == self._frame_count_px_px - 1:
                    ...
                    tifffile.imwrite(Path(self.path, self._acquisition_name, f"{self.filename}_mean_projection_xy_z_{start_index:06}_{end_index:06}.tiff"), self.mip_xy)
                    # reset the xy mip
                    self.mip_xy = np.zeros((self._row_count_px, self._column_count_px), dtype=self._data_type)
                frame_index += 1
                self.new_image.clear()

        tifffile.imwrite(Path(self.path, self._acquisition_name, f"{self.filename}_mean_projection_yz.tiff"), self.mip_yz)
        tifffile.imwrite(Path(self.path, self._acquisition_name, f"{self.filename}_mean_projection_xz.tiff"), self.mip_xz)
