...
class GPUToolsDownSample2D(BaseDownSample):
    ...
    def __init__(self, binning: int):
        ...
        self._prog = OCLProgram(
            src_str=self._kernel, build_options=["-D", f"BLOCK={self._block_size}"]
        )
    ...
    def run(self, image: numpy.array):
        ...
        y_g = OCLArray.empty(
            tuple(s // self._block_size for s in image.shape), image.dtype
        )
        self._prog.run_kernel(
            f"downsample2d", y_g.shape[::-1], None, x_g.data, y_g.data
        )
        return y_g.get()
