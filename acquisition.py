...
class Acquisition():
    ...
    def run(self):
        ...
        for tile in acquisition['tiles']:
            ...
            for camera_id in self.instrument.cameras:
                ...
                self.instrument.cameras[camera_id]['object'].prepare()
    ...
    def check_disk_space(self):
        ...
        for camera_id in self.instrument.cameras:
            for tile in self.config.cfg['acquisition']['tiles']:
                row_count = self.instrument.cameras[camera_id]['object'].roi['height_px']
                column_count = self.instrument.cameras[camera_id]['object'].roi['width_px']
                data_type = self.instrument.cameras[camera_id]['writer']['object'].data_type
                frame_count = tile['frame_count']
                data_size_gb += row_count*column_count*frame_count*numpy.dtype(data_type).itemsize/1e9

        self.log.info(f'required disk space = {data_size_gb} [GB]')

        free_size_gb = shutil.disk_usage(local_drive).free/1e9
        self.log.info(f'available disk space = {free_size_gb} [GB] on local drive {local_drive}')

        if data_size_gb >= free_size_gb:
            self.log.error(f"not enough space in local directory: {local_drive}")
            raise

        free_size_gb = shutil.disk_usage(external_drive).free/1e9
        self.log.info(f'available disk space = {free_size_gb} [GB] on external drive {external_drive}')
    ...
    def check_write_speed(self, size='16Gb', bs='1M', direct=1, numjobs=1, ioengine= 'windowsaio',
        ...
        for drive in [local_drive, external_drive]:
            test_filename = Path(f'{drive}\\test.txt')
            print(test_filename)
            ...
            try:
                ...
                self.log.info(f'available write speed = {write_speed_mb_s} [MB/sec] on drive {drive}')
    ...
    def check_system_memory(self):
        ...
        for camera_id in self.instrument.cameras:
            ...
            column_count = self.instrument.cameras[camera_id]['object'].roi['width_px']
            data_type = self.instrument.cameras[camera_id]['writer']['object'].data_type
            chunk_count = self.instrument.cameras[camera_id]['writer']['object'].chunk_count
